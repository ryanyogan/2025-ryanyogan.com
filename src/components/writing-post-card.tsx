    ...
    <article className="group pb-6 max-w-3xl">
      <Link href={`/writing/${post.slug ?? ""}`} className="flex flex-col">
        ...
        <div className="flex justify-between items-center pb-1">
          <h3 className="text-base font-sans font-medium text-blue-600 underline group-hover:text-blue-800 transition-colors line-clamp-1">
            {post.title}
          </h3>
        </div>
        <p className="text-base font-sans text-zinc-800 line-clamp-1 tracking-tight">
          {post.excerpt}
        </p>
      </Link>
    </article>
